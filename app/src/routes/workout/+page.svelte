<script lang="ts">
	...
	const ACCENT = new Set([
		...
		'left_ear', 'right_ear', 'nose'  // useful for head exercises
	])

	// callback (called by ml5 per frame)
	function onPoses(poses: any[]) {
		...
		for (const [a, b] of EDGES) {
			...
			if (ka?.confidence > 0.25 && kb?.confidence > 0.25) {
				ctx.beginPath()
	...
	// Stop the camera and ML model when the user logs out mid-session
	$effect(() => {
		if (!$wallet.connected) teardown()
	})
